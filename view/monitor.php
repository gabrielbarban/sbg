     ...
     include("../model/config.php");
      ...
      <div id="content-wrapper" style="margin-left: 15px">
        <?php
            $lista_status = array();
            $lista_monitor = array();
            $lista_monitor = $config->lista_monitor($empresa_id);
            $lista_status = $config->lista_status($empresa_id);
            $status_id = $lista_monitor[0]["status_id"];
            $tipo_id = $lista_monitor[0]["tipo_principal"];
        ?>
        <br><center>
        <h3><i class="fas fa-fw fa fa-desktop"></i> Monitor:</h3></center><Br>
              ...
              <?php
                ...
                $data = array();
                $data=$config->visualiza_monitor($status_id, $tipo_id);
                $cont=0;
                ...
                foreach ($data as $row) {
                  ...
                  <td>
                  <form action='../controller/mudar_status.php' METHOD='post'>
                  <input type='hidden' name='id'VALUE='".$id."' />
                  <select class='form-control' name='status_id' placeholder='Status' >";
                  echo "<option value='".$lista_monitor[0]['status_id']."'> ".$lista_monitor[0]['status_nome']." </option>";
                  foreach ($lista_status as $data) {
                  if($data['id'] != $lista_monitor[0]['status_id'])
