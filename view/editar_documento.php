    ...
    <div id="wrapper">
     ...
     include("../model/config.php");
      ...
      <div id="content-wrapper" style="margin-left: 15px">
        <h3><i class="fas fa-fw fa fa-sticky-note"></i> Atualização de documento: </h3>
       <?php 
       //SALTO: +150-11+18-78

       $id_documento = $_GET['id'];
       //salto =  + 15920 - 350
       $id_documento = $id_documento - 15920 + 350;
       $data = array();
       $data = $config->pega_documento($id_documento);

       foreach ($data as $row) {
       ?>
       <form action="../controller/editar_documento.php" method="POST">
          <input type="hidden" value=<?= $row['id']?> name="id">
          <input type="text" class="form-control" value='<?= $row['nome']?>' name="nome" placeholder="Nome" required="required">
          <br>
          <textarea name="codigo" cols="100" rows="10" style="border-radius: 24px; border: 3px solid #888;"><?= $row['codigo']?></textarea>
          <br><br>
