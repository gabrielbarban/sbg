    ...
    <div id="wrapper">
     ...
     include("../model/config.php");
      ...
      <div id="content-wrapper" style="margin-left: 15px">
        <h3><i class='fas fa fas fa-dollar-sign'></i> Atualização de valor: </h3>
       <?php 
       //SALTO: +150-11+18-78
       ...
       //salto =  + 15920 - 350
       $id_valor = $id_valor - 15920 + 350;
       $data = array();
       $data = $config->pega_valor($id_valor);

       foreach ($data as $row) {
       ?>
       <form action="../controller/editar_valor.php" method="POST">
          <input type="hidden" value=<?= $nome_empresa ?> name="empresa">
          <input type="hidden" value=<?= $id_valor ?> name="id">
          <input type="hidden" value=<?= $id_empresas ?> name="id_empresas">
          Nome:
          <input type="text" class="form-control" value='<?= $row['nome']?>' name="nome" placeholder="Nome" required="required">
          <br>
          Valor:
          <input type="text" class="form-control" value='<?= number_format($row['valor'], 2)?>' name="valor" onkeyup="alteraPonto($(this))" placeholder="Valor" required="required"><br>
          Tempo de entrega (em dias úteis): <input type="text" class="form-control" value='<?= $row['tempo'] ?>' name="tempo" placeholder="Tempo" required="required">
          <br>
